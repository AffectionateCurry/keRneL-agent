...
def train(run_folder: str, output_dir: str):
    ...
    cmd = (
        ...
        f"{torch.cuda.device_count()} --num_machines 1 --mixed_precision no "
        "--dynamo_backend no -m axolotl.cli.train ./config.yml "
        + ("--wandb_mode disabled" if not ALLOW_WANDB else "")
    )
    ...
    with open(f"{run_folder}/logs.txt", "a", encoding="utf-8") as f:
...
def merge(run_folder: str, output_dir: str):
    ...
    with open(f"{run_folder}/config.yml", encoding="utf-8"):
        print(f"Merge from {output_path}")

    MERGE_CMD = (
        "accelerate launch --num_processes "
        f"{torch.cuda.device_count()} --num_machines 1 --mixed_precision no "
        "--dynamo_backend no -m axolotl.cli.merge_lora ./config.yml "
        f"--lora_model_dir='{output_dir}'"
    )
...
def launch(config_raw: str, data_raw: str, run_to_resume: str, preproc_only: bool):
    ...
    run_name = (
        f"axo-{time_string}-{secrets.token_hex(2)}" if not run_to_resume else run_to_resume
    )
    ...
    with (
        open(f"{run_folder}/config.yml", "w", encoding="utf-8") as config_file,
        open(
            f"{run_folder}/{config['datasets'][0]['path']}", "w", encoding="utf-8"
        ) as data_file,
    ):
    ...
    if preproc_only:
        ...
        with open(f"{run_folder}/logs.txt", "w", encoding="utf-8") as f:
            lbl = "preproc"
...
def main(
    ...
    """Local CLI entrypoint. Passes config & data files to remote launch()."""

    # Read config and data source files and pass their contents to the remote function.
    with open(config, "r", encoding="utf-8") as cfg, open(
    ...
    if not preproc_only:
        print(
            "To run sample inference, run `modal run --quiet -m src.inference --run-name "
            f"{run_name}`"
        )


def run_cmd(cmd: str, run_folder: str):
    """Run a command inside a folder, reloading Modal Volumes before and committing on success."""
    ...
    if exit_code := subprocess.call(shlex.split(cmd), cwd=run_folder):
        exit(exit_code)
